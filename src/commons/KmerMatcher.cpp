    ...
    for (size_t i = 0; i < querySplits.size(); i++) {
        ...
        for (size_t j = querySplits[i].start; j < querySplits[i].end + 1; j++) {
            ...
            if (currentQuery == queryKmerList[j].ADkmer
                ...
                //  cout << "Identical Query";
                //  seqIterator->printKmerInDNAsequence(queryKmerList[j].ADkmer); cout << " "; seqIterator->printKmerInDNAsequence(currentQuery); cout << endl;
                for (size_t k = 0; k < selectedMatchCnt; k++) {
                    // if (par.threads == 1) {
                    //     seqIterator->printKmerInDNAsequence(queryKmerList[j].ADkmer); cout << " ";
                    //     seqIterator->printKmerInDNAsequence(decodedKmerBuffer[selectedMatches[k] + matchStartIdx]); cout << endl;
                    // }
            ...
            if (currentQueryAA == AMINO_ACID_PART(queryKmerList[j].ADkmer)) {
                compareDna3(queryKmerList[j].ADkmer, decodedKmerBuffer, matchStartIdx, aaMatchCnt, hammingDists, selectedMatches, selectedHammingSum,
            ...
            // Skip target k-mers not matching in AA
            while (true) {
            ...
            while (true) {
                while (decodedKmerBufferIdx < decodedKmerCnt 
                    && currentQueryAA == AMINO_ACID_PART(decodedKmerBuffer[decodedKmerBufferIdx])) {
                    // seqIterator->printAAKmer(currentQuery, 24); cout << " "; seqIterator->printAAKmer(decodedKmerBuffer[decodedKmerBufferIdx], 24); cout << endl;
            ...
            compareDna3(currentQuery, decodedKmerBuffer, matchStartIdx, aaMatchCnt, hammingDists, selectedMatches, selectedHammingSum,
                       selectedHammings, selectedDnaEncodings, selectedMatchCnt, queryKmerList[j].info.frame);
...
void KmerMatcher::compareDna3(uint64_t query,
                             ...
                             size_t & selectedMatchIdx,
                             uint8_t frame) {
    hammingDists.resize(aaMatchCnt);
    uint8_t minHammingSum = UINT8_MAX;

    // Calculate hamming distance
    for (size_t i = 0; i < aaMatchCnt; i++) {
