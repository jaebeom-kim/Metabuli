        ...
        for (size_t j = querySplits[i].start; j < querySplits[i].end + 1; j++) {
            ...
            if (currentQuery == queryKmerList[j].ADkmer
                ...
                for (size_t k = 0; k < selectedMatchCnt; k++) {
                    ...
                    if (candidateKmerInfos[idx] == -2144786599 || taxId2speciesId[candidateKmerInfos[idx]] == -2144786599) {
                        std::cout << "Error: " << candidateKmerInfos[idx] << " " << taxId2speciesId[candidateKmerInfos[idx]] << std::endl;
                    }
                    matchCnt++;
                }
                continue;
