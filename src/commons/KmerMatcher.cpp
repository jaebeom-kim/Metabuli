...
KmerMatcher::KmerMatcher(const LocalParameters & par,
    ...
    dnaEncodingMasker = 16777215; 
    dnaEncodingMasker = ~ dnaEncodingMasker;
    protIdMasker = ~((1ULL << 28) - 1);
    protKmerMasker = (1ULL << 28) - 1;
    totalMatchCnt = 0;

    this->taxonomy = taxonomy;
    if (par.proteinDB.empty()) { // When searching UniRef90, taxID list is not needed.
        loadTaxIdList(par);
    }
...
bool KmerMatcher::matchKmers(Buffer<QueryKmer> * queryKmerBuffer,
    ...
    std::vector<QueryKmerSplit<DiffIdxSplit>> querySplits;
    uint64_t queryAA;
...
querySplits, queryKmerList, matchBuffer, cout, targetDiffIdxFileName, numOfDiffIdx, targetInfoFileName)
        {
            // FILE
            FILE * diffIdxFp = fopen(targetDiffIdxFileName.c_str(), "rb");
            FILE * kmerIdFp = fopen(targetInfoFileName.c_str(), "rb");

            // Target K-mer buffer
            uint16_t * diffIdxBuffer = (uint16_t *) malloc(sizeof(uint16_t) * (BufferSize + 1)); // size = 32 Mb
            uint32_t * kmerIdBuffer = (uint32_t *) malloc(sizeof(uint32_t) * (BufferSize + 1)); // 64 Mb
            size_t kmerIdBufferIdx = 0;
            size_t diffIdxBufferIdx = 0;
            ...
            size_t diffIdxPos = 0;
            std::vector<uint64_t> candidateTargetKmers; //vector for candidate target k-mer, some of which are selected after based on hamming distance
            std::vector<uint32_t> candidateKmerIds;
            uint64_t currentTargetKmer;

            //Match buffer for each thread
            int localBufferSize = 2'000'000; // 32 Mb
            auto *matches = new MatchF[localBufferSize]; // 16 * 2'000'000 = 32 Mb
            int matchCnt = 0;
...
#pragma omp for schedule(dynamic, 1)
            for (size_t i = 0; i < querySplits.size(); i++) {
                ...
                currentTargetKmer = querySplits[i].diffIdxSplit.ADkmer;
                diffIdxBufferIdx = querySplits[i].diffIdxSplit.diffIdxOffset;
                kmerIdBufferIdx = querySplits[i].diffIdxSplit.infoIdxOffset
                                    - (querySplits[i].diffIdxSplit.ADkmer != 0);
                diffIdxPos = querySplits[i].diffIdxSplit.diffIdxOffset;

                fseek(kmerIdFp, 4 * (long)(kmerIdBufferIdx), SEEK_SET);
                loadBuffer(kmerIdFp, kmerIdBuffer, kmerIdBufferIdx, BufferSize);
                fseek(diffIdxFp, 2 * (long) (diffIdxBufferIdx), SEEK_SET);
                loadBuffer(diffIdxFp, diffIdxBuffer, diffIdxBufferIdx, BufferSize);
                ...
                for (size_t j = querySplits[i].start; j < querySplits[i].end + 1; j++) {
                    ...
                    if (currentQuery == queryKmerList[j].ADkmer
                        ...
                        for (int k = 0; k < currMatchNum; k++) {
                            ...
                            // }
                            matches[matchCnt] = {queryKmerList[j].info,
                                                 candidateKmerIds[idx],
                                                 taxId2speciesId[candidateKmerIds[idx]],
                                                 selectedDnaEncodings[k],
                                                 selectedHammings[k],
                                                 selectedHammingSum[k]};
                            matchCnt++;
                    ...
                    while (diffIdxPos != numOfDiffIdx &&
                           currentQueryAA == AminoAcidPart(currentTargetKmer)) {
                        candidateTargetKmers.push_back(currentTargetKmer);
                        candidateKmerIds.push_back(getKmerId(BufferSize, kmerIdFp, kmerIdBuffer, kmerIdBufferIdx));
                        // Print the target k-mer
//                        if (par.printLog == 1) {
//                            cout << queryKmerList[j].info.sequenceID << "\t" << queryKmerList[j].info.pos << "\t"
//                                 << (int) queryKmerList[j].info.frame << endl;
...
bool KmerMatcher::matchAAKmers(Buffer<ExtractedMetamer> * queryKmerBuffer,
                               ...
                               const string & db) {
    cout << "Searching UniRef90 database..." << endl;
    ...
    for (size_t i = 0; i < threads; i++) {
        ...
        if (needLastTargetBlock) {
            querySplits.emplace_back(startIdx, endIdx, endIdx - startIdx + 1, deltaIdxSplits.data[numOfDiffIdxSplits_use - 2]); // error
        }
        startIdx = endIdx + 1;
    }
    ...
    while (completedSplitCnt < threads) {
        bool hasOverflow = false;
#pragma omp parallel default(none), shared(completedSplitCnt, splitCheckList, hasOverflow, \
querySplits, queryKmerList, matchBuffer, cout, targetDiffIdxFileName, numOfDeltas, targetInfoFileName, spTaxId2UniRefTaxIds, unirefIdx2taxId, ncbi2gtdb)
        {
            ...
            //Match buffer for each thread
            int localBufferSize = 3'000'000; 
            auto *localMatches = new ProtMatch[localBufferSize]; // 12 * 3'000'000 = 36 Mb
            int matchCnt = 0;
            size_t posToWrite;
                ...
                for (size_t j = querySplits[i].start; j < querySplits[i].end + 1; j++) {
                    ...
                    if (currentQueryAA == AminoAcidPart(queryKmerList[j].metamer.metamer)) {
                        ...
                        if (matchCnt + currMatchNum > localBufferSize) {
                            // Check if the shared buffer is full.
                            posToWrite = matchBuffer->reserveMemory(matchCnt);
                            if (posToWrite + matchCnt >= matchBuffer->bufferSize) { // full
                                hasOverflow = true;
                                __sync_fetch_and_sub(&matchBuffer->startIndexOfReserve, matchCnt);
                                break;
                            } else { // not full -> copy matches to the shared buffer
                                moveMatches(matchBuffer->buffer + posToWrite, localMatches, matchCnt);
                            }
                        }
                        if (ncbi2gtdb.empty()) {
                            for (int k = 0; k < currMatchNum; k++) {
                                uint32_t targetKmerId = getIdOfAAkmer(aaMatches[k]);
                                TaxID ncbiTaxId = unirefIdx2taxId.at(targetKmerId);
                                if (spTaxId2UniRefTaxIds.at(queryKmerList[j].speciesId).find(ncbiTaxId) 
                                 != spTaxId2UniRefTaxIds.at(queryKmerList[j].speciesId).end()) {
                            ...
                            for (int k = 0; k < currMatchNum; k++) {
                                uint32_t targetKmerId = getIdOfAAkmer(aaMatches[k]);
                                TaxID ncbiTaxId = unirefIdx2taxId.at(targetKmerId);
                                if (ncbi2gtdb.find(ncbiTaxId) != ncbi2gtdb.end()) {
                                    TaxID gtdbTaxId = ncbi2gtdb.at(ncbiTaxId);
                                    if (spTaxId2UniRefTaxIds.at(queryKmerList[j].speciesId).find(gtdbTaxId) 
                                     != spTaxId2UniRefTaxIds.at(queryKmerList[j].speciesId).end()) {
                                        localMatches[matchCnt++] = {queryKmerList[j].metamer.id,
                                                                   queryKmerList[j].cdsPos, // cdsId
                                                                   targetKmerId}; // protId
...
void KmerMatcher::sortMatches(Buffer<MatchF> * matchBuffer) {
    time_t beforeSortMatches = time(nullptr);
    std::cout << "Sorting matches ..." << std::endl;
    SORT_PARALLEL(matchBuffer->buffer,
                  matchBuffer->buffer + matchBuffer->startIndexOfReserve,
...
void KmerMatcher::moveMatches(MatchType *dest, MatchType *src, int &matchNum) {
    memcpy(dest, src, sizeof(MatchType) * matchNum);
    matchNum = 0;
}
...
bool KmerMatcher::compareMatchF(const MatchF & a, const MatchF & b) {
    ...
    if (a.totalHammingDist != b.totalHammingDist)
        return a.totalHammingDist < b.totalHammingDist;
