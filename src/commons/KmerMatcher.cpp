...
bool KmerMatcher::matchAAKmers(Buffer<TargetMetamerF> * queryKmerBuffer,
    ...
    size_t queryKmerNum = queryKmerBuffer->startIndexOfReserve;
    TargetMetamerF *queryKmerList = queryKmerBuffer->buffer;
    
    // Find the first index of garbage query k-mer (UINT64_MAX) and discard from there
    for (size_t checkN = queryKmerNum - 1; checkN > 0; checkN--) {
        if (queryKmerList[checkN].metamerF.metamer != UINT64_MAX) {
            queryKmerNum = checkN + 1;
            break;
        }
    }
    ...
    for (size_t i = 0; i < threads; i++) {
        ...
        bool needLastTargetBlock = true;
        queryAA = AminoAcidPart(queryKmerList[startIdx].metamerF.metamer);
        for (size_t j = 0; j < numOfDiffIdxSplits_use; j ++) {
    ...
    while (completedSplitCnt < threads) {
        bool hasOverflow = false;
#pragma omp parallel default(none), shared(completedSplitCnt, splitCheckList, hasOverflow, \
querySplits, queryKmerList, matchBuffer, cout, targetDiffIdxFileName, numOfDeltas, targetInfoFileName)
        {
...
#pragma omp for schedule(dynamic, 1)
            for (size_t i = 0; i < querySplits.size(); i++) {
                ...
                size_t lastMovedQueryIdx = 0;
                const TargetMetamerF * curMetamer;
                for (size_t j = querySplits[i].start; j < querySplits[i].end + 1; j++) {
                    querySplits[i].start++;
                    // Skip metamers from non-CDS
                    if (queryKmerList[j].metamerF.protId == 0) {
                        continue;
                    }

                    // Reuse the amino acid matches 
                    if (currentQueryAA == AminoAcidPart(queryKmerList[j].metamerF.metamer)) {
                        // If local buffer is full, copy them to the shared buffer.
                        ...
                        for (int k = 0; k < currMatchNum; k++) {
                            matches[matchCnt++] = {queryKmerList[j].metamerF.protId,
                    ...
                    currentQueryAA = AminoAcidPart(queryKmerList[j].metamerF.metamer);
...
// void KmerMatcher::moveMatches(ProtMatch *dest, ProtMatch *src, int &matchNum) {
//     memcpy(dest, src, sizeof(ProtMatch) * matchNum);
//     matchNum = 0;
// }
