...
public:
    ...
    template<typename T>
    static T getQueryKmerNumber(T queryLength, int kLength = 8);
...
template <typename T>
T LocalUtil::getQueryKmerNumber(T queryLength, int kLength) {
    return (getMaxCoveredLength(queryLength) / 3 - kLength + 1) * 6;
}
